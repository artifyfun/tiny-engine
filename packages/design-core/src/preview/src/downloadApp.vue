<template>
  <div class="preview-box">
    <div class="preview">
      <div class="preview-title">温馨提示：正在构建应用, 请耐心等待, 应用构建完成后自动下载到本地</div>
      <tiny-progress
        class="progress"
        :text-inside="true"
        :stroke-width="30"
    ...
    const cyclesFetchStatus = (taskId) => {
      ...
      http.get(`/app-center/api/tasks/status/${taskId}`).then((data) => {
        ...
        if (data.taskStatus === taskStatus.RUNNING) {
          ...
          updateApp(appId).then(() => {
            if (appInfoState.selectedApp?.assets_url) {
              window.location = appInfoState.selectedApp?.assets_url
            }
